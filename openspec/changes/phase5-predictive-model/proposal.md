...
Every capability in MARTA so far is retrospective: it classifies and quantifies change that already happened (Fase 1: detection, Fase 2: carbon for observed change). The tutor separately suggested, in conversation, that the project also produce a forward-looking view — given the change already observed in the AOI's historical record, predict how the terrain is likely to behave next, and what CO2e that implies. This isn't part of his originally documented 6-phase methodology (`research_docs/METODOLOGIA_TUTOR_RODRIGO_PARRA.md`, entirely stock-based/retrospective), so it's tracked here as an explicit, separately-scoped addition rather than folded into an existing phase — including the open question of whether it's required core or optional, which still needs his confirmation.
