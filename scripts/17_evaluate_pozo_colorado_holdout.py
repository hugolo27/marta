"""Cross-region generalization check #3, phase1-classifier task 5.7-5.9 (Pozo Colorado,
Presidente Hayes). Mirrors 14_evaluate_filadelfia_holdout.py / 15_evaluate_bahia_negra_holdout.py
exactly, over a third held-out sub-region: cattle-ranching area on the Trans-Chaco highway,
pasture-dominated — a distinct landscape profile from both Filadelfia (agricultural colonies)
and Bahia Negra (remote forest), so the CNN's bosque-specific failure can be checked against a
region where bosque itself is scarce and pasto dominates.

Usage: .venv/bin/python scripts/17_evaluate_pozo_colorado_holdout.py"""
...
OUT_PATH = REPO_ROOT / "data" / "study_area" / "pozo_colorado_holdout_results.json"
...
def extract_points(heldout):
    ...
    for period, (start, end) in PERIODS.items():
        ...
        # here too (Pozo Colorado doesn't straddle a UTM boundary the way Filadelfia does, but
        # there's no correctness cost to leaving it off, and it keeps this script consistent
        # with 14_.../15_... rather than re-deriving per-region reasoning each time).
...
def main():
    ...
    print(f"\nRunning {len(holdout_eval.SEEDS)}-seed RF/CNN comparison over Pozo Colorado "
    ...
    multiseed = holdout_eval.run_multiseed(rows, y_true, "Pozo Colorado")
    ...
    with mlflow.start_run(run_name="pozo_colorado_holdout_5seed"):
