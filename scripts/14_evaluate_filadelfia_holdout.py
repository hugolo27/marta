"""Cross-region generalization check, phase1-classifier tasks 5.1-5.3. Runs the operational
classifier (CNN, task 4.3) plus Random Forest for reference over the Filadelfia held-out
sub-region, never touched by training/val/test — same MapBiomas-derived ground truth as the AOI
test split (so accuracy is directly comparable), plus an independent ESA WorldCover cross-check.

Sampling mirrors 08_assemble_dataset.py's approach (stratifiedSample + batched sampleRegions with
neighborhoodToArray) rather than dense wall-to-wall inference over the full ~65,000ha box, which
would mean a 40km x 40km raster at 10m (~16M pixels) pulled through getInfo() - infeasible the
same way a single sampleRectangle() call would be. A stratified point sample is the same accuracy-
assessment approach already used to build and evaluate the training dataset itself.

Usage: .venv/bin/python scripts/14_evaluate_filadelfia_holdout.py"""
...
OUT_PATH = REPO_ROOT / "data" / "study_area" / "filadelfia_holdout_results.json"
...
N_PER_CLASS = 60  # per period; ~360 target points total, same order of magnitude as the 179-patch AOI test split
BATCH_SIZE = 25
...
# WorldCover legend remap: imported from 07_extract_worldcover.py rather than retyped, so the
# two scripts can't silently disagree if the mapping is ever corrected in one place (code review
# 2026-09-11).
...
def extract_points(heldout):
    ...
    for period, (start, end) in PERIODS.items():
        # reproject=False: the Filadelfia box straddles the UTM 20S/21S boundary at -60 degrees,
        # same multi-zone condition 08_assemble_dataset.py guards against (see s2_utils
        # docstring) — reproject=True here would distort pixels on one side of that meridian
        # before scoring (code review 2026-09-11).
...
def main():
    ...
    print(f"\nRunning {len(holdout_eval.SEEDS)}-seed RF/CNN comparison over Filadelfia "
    ...
    multiseed = holdout_eval.run_multiseed(rows, y_true, "Filadelfia")
    ...
    # Read live rather than hardcode: this file gets regenerated by 11_compare_classifiers.py and
    # a hardcoded copy here would go stale every time it does (code review 2026-09-11).
    ...
    with mlflow.start_run(run_name="filadelfia_holdout_5seed"):
